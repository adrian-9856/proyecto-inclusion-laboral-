...
function normalizarGeneroTech(g) {
  const mapa = {
...
function aplicarColorEstadoFila(sheet, fila, estado) {
  const color = CONFIG_TECH.COLORES_ESTADO[estado] || null;
...
function setupMenuTech() {
  try {
    ...
    ui.createMenu('💻 Tecnología')
      ...
      .addItem('🛠️ ACTUALIZAR COLUMNAS Y DESPLEGABLES', 'actualizarColumnasYDesplegablesTech')
      .addItem('🔧 APLICAR MEJORAS 2026', 'aplicarMejoras2026Tech')
      .addSeparator()
...
function importarDesdeKoboInterno(ss, ui, url, tipoImportacion) {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const genero = normalizarGeneroTech(colIndices.genero >= 0 ? fila[colIndices.genero].toString().trim() : '');
...
function _pbiAnio(v) {
  const d = _pbiFechaObj(v);
  return d ? d.getFullYear() : '';
}

function _pbiMesNum(v) {
  const d = _pbiFechaObj(v);
  return d ? d.getMonth() + 1 : '';
}

function _pbiNombreMes(v) {
  const d = _pbiFechaObj(v);
  return d ? _MESES_NOMBRES[d.getMonth()] : '';
}

function _pbiRangoEdad(edad) {
...
function _pbiDiasEntre(f1, f2) {
  const d1 = _pbiFechaObj(f1);
  const d2 = _pbiFechaObj(f2);
  if (!d1 || !d2) return '';
  const diff = Math.round((d2 - d1) / (1000 * 60 * 60 * 24));
  return diff >= 0 ? diff : '';
}

function _pbiTrimestre(v) {
  const d = _pbiFechaObj(v);
  return d ? Math.ceil((d.getMonth() + 1) / 3) : '';
}

function _pbiTrimestrePeriodo(v) {
  const d = _pbiFechaObj(v);
  if (!d) return '';
  return d.getFullYear() + '-Q' + Math.ceil((d.getMonth() + 1) / 3);
}

function _pbiMesAnio(v) {
  const d = _pbiFechaObj(v);
  return d ? _MESES_NOMBRES[d.getMonth()].substring(0, 3) + '-' + d.getFullYear() : '';
}

function _pbiEspecialidad(cohorte) {
  const c = (cohorte || '').toString().toLowerCase();
  if (c.includes('sac'))         return 'SAC';
  if (c.includes('computaci'))   return 'Computación';
  if (c.includes('cocina'))      return 'Cocina';
  if (c.includes('reposteri'))   return 'Repostería';
  if (c.includes('barismo'))     return 'Barismo';
  if (c.includes('pasteleri'))   return 'Pastelería';
  if (c.includes('panaderi'))    return 'Panadería';
  return cohorte ? 'Otro' : '';
}
...
function actualizarPowerBIExport() {
  ...
  try {
    ...
    for (let i = 1; i < dInt.length; i++) {
      ...
      if (!cId) continue;
      // Filtrar: solo participantes de programas de Tecnología
      if (c.int.prog >= 0) {
        const prog = (row[c.int.prog] || '').toString().trim().toLowerCase();
        if (!CONFIG_TECH.PROGRAMAS_TECNOLOGIA.some(p => prog.includes(p.toLowerCase()))) continue;
      }
 ...
 */
function actualizarColumnasYDesplegablesTech() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const ui = SpreadsheetApp.getUi();
  ...
  try {
    ...
    // Asegurar que Notas/Comentario exista antes de mover datos de Responsable
    let idxNotas = headers.findIndex(h => h.toString().trim() === 'Notas/Comentario');
    if (idxNotas < 0) {
    ...
    // Mover contenido de Responsable → Notas/Comentario
    const idxResponsable = headers.findIndex(h => h.toString().trim() === 'Responsable');
    if (idxResponsable >= 0 && lastRow > 1) {
      const respData = sheet.getRange(2, idxResponsable + 1, lastRow - 1, 1).getValues();
      const notasData = sheet.getRange(2, idxNotas + 1, lastRow - 1, 1).getValues();
    ...
    // Eliminar columnas (de mayor a menor para no desfasar)
    const headersActuales = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
...
function limpiarNotaRepetidaTech() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const ui = SpreadsheetApp.getUi();
  const sheet = ss.getSheetByName('Hoja de Interés');
  if (!sheet) { ui.alert('No se encontró la Hoja de Interés.'); return; }
  ...
  // Contar cuántas veces aparece cada texto (solo textos con >1 aparición)
  const conteo = {};
  notas.forEach(r => {
  ...
  // Mostrar al usuario los duplicados encontrados
  ...
  // Limpiar duplicados: mantener solo la primera aparición
...
function aplicarMejoras2026Tech() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const ui = SpreadsheetApp.getUi();
