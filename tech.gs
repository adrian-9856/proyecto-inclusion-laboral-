...
function configurarValidaciones() {
  ...
  if (seleccionadas) {
    ...
    seleccionadas.getRange('N2:N500').setDataValidation(
      SpreadsheetApp.newDataValidation().requireValueInList(['Sí, trasladar a A y B']).setAllowInvalid(false).build()
    );
  }
...
function activarMejorasEntrevistasTech() {
  ...
  if (entrevistas) {
    ...
    if (colEstado > 0) {
      const estadoOpciones = ['Aprobada', 'No asistió / No aprobó', 'Reprogramada', 'Próxima cohorte Programación', 'Próxima cohorte Alfa Digital', 'Derivar a Paso a Paso', '🔗 Abrir Formulario', 'Enviar a A y B'];
